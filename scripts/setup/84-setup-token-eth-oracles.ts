...
export async function setupTokenEthOracles(
  ...
  const oracleConfig = USDC_ETH_ORACLE_CONFIG;
  ...
  if (!oracleAddress) {
    throw new Error(
      `Missing deployment address ${oracleConfig.oracleAddressKey}`,
    );
  }

  if (!tokenAddress) {
    throw new Error(
      `Missing deployment address ${oracleConfig.tokenAddressKey}`,
    );
  }
  ...
  for (const context of TOKEN_ETH_CONTEXTS) {
    ...
    if (
      !(await priceManager.tokenAllowedForContext(tokenAddress, context.id))
    ) {
    ...
    registeredContexts: Object.fromEntries(
      await Promise.all(
        TOKEN_ETH_CONTEXTS.map(async (context) => [
          context.name,
          await priceManager.getOraclesForTokenContext(
            tokenAddress,
            context.id,
          ),
        ]),
